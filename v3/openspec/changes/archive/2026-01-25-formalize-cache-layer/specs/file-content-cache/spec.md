...
**Capability ID**: `file-content-cache`
**Version**: 1.0
**Status**: Implemented

## Overview

The file content cache converts sequential ZIP streams into random-access storage, enabling Windows file system operations that require arbitrary offset reads. It uses a generic cache implementation with pluggable storage strategies and eviction policies.
...
Cache lookups SHALL be lock-free for maximum performance. The fast path SHALL use `ConcurrentDictionary.TryGetValue()` with no locking.
...
The cache SHALL expose real-time metrics for monitoring and debugging.
...
|----------|------|---------|-------------|
| `MemoryCacheSizeMb` | int | 2048 | Memory tier capacity in MB |
| `DiskCacheSizeMb` | int | 10240 | Disk tier capacity in MB |
| `SmallFileCutoffMb` | int | 50 | Routing threshold in MB |
| `TempDirectory` | string? | null | Disk tier temp directory (null = system temp) |
| `DefaultTtlMinutes` | int | 30 | Default entry TTL in minutes |
| `EvictionCheckIntervalSeconds` | int | 60 | Background cleanup interval |

---

## Related Specifications

- `archive-structure-cache` - Caching of parsed ZIP metadata (uses ObjectStorageStrategy)
- `streaming-zip-reader` - Source of data to be cached

---

## Implementation Notes

- **Concurrency Model**: Three-layer strategy (lock-free → per-key → eviction lock)
- **Storage Abstraction**: `IStorageStrategy<T>` with `Store`, `Retrieve`, `Dispose` methods
- **Entry Protection**: RAII pattern via `ICacheHandle<T>` with automatic RefCount management

...
public class ShellMetadataFilterTests
{
    ...
    [InlineData("/archive.zip\\subfolder/thumbs.db")]
    [InlineData(@"\archive.zip\Desktop.INI")]
    [InlineData(@"\archive.zip\DESKTOP.INI")]
    ...
    [InlineData("/$RECYCLE.BIN/")]
    [InlineData("//$RECYCLE.BIN/tracking.log")]
    ...
    [InlineData(@"\archive.zip\desktop.ini.bak")]
    [InlineData("desktop.ini.bak")]
    [InlineData(@"\archive.zip\mythumbs.db")]

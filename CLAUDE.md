...
- **Windows x64 and ARM64** - Uses WinFsp which is Windows-specific
- **WinFsp 2.x** must be installed from https://winfsp.dev/rel/
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  1. WinFsp.Native: ReadFile("R:\archive.zip\folder\file.txt", offset=5000)  │
│     ↓                                                                       │
...
│     ↓                                                                       │
│  6. Stream: Seek(5000), Read(4096) → Return to WinFsp.Native                │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
WinFsp integration for Windows file system mounting. The managed binding is the NuGet package `WinFsp.Native` (https://github.com/hooyao/winfsp-native); the native `winfsp-x64.dll` / `winfsp-a64.dll` is provided by the system WinFsp install.

**Key Components**:
- `WinFspFileSystemAdapter`: Implements `WinFsp.Native`'s `IFileSystem` interface, translates WinFsp callbacks to `IVirtualFileSystem`
- `WinFspHostedService`: `IHostedService` that manages mount/unmount lifecycle
- `WinFspTelemetry`: Static `Meter("ZipDrive.WinFsp")` with read latency histogram
...
- **WinFspFileSystemAdapter.Guarded*Async**: Five public async methods for test consumption without the WinFsp runtime (`GuardedReadFileAsync`, `GuardedListDirectoryAsync`, `GuardedGetFileInfoAsync`, `GuardedFileExistsAsync`, `GuardedDirectoryExistsAsync`).

**ReadFile Zero-Copy**: `WinFspFileSystemAdapter.ReadFile()` receives WinFsp's kernel buffer as a `Memory<byte>` (exactly the requested length) and passes it straight to `IVirtualFileSystem.ReadFileAsync(string, Memory<byte>, long, ...)`, which writes decompressed bytes directly into it — no `ArrayPool` rent and no intermediate `memcpy` per read. The returned byte count never exceeds `buffer.Length` (the cache caps it).
...
**Unmanaged Memory**: A FUSE-like file system on Windows carries a baseline of unmanaged memory from the WinFsp kernel driver and its user-mode communication buffers (loaded via the `winfsp-x64.dll` / `winfsp-a64.dll` native library), not from ZipDrive's managed code. The managed heap is typically ~1-2MB. This is the normal cost of a user-mode file system and is outside our control.
...
| **Static Telemetry** | `CacheTelemetry`, `ZipTelemetry`, `WinFspTelemetry` | Zero-DI metrics/tracing |

# chunked-disk-extraction Specification

## Purpose
Incremental chunk-based decompression for disk-tier cache entries. Reduces first-byte latency from seconds to ~50ms by extracting in fixed-size chunks and serving completed chunks while extraction continues in background.
...
- **AND** the chunk's ready flag SHALL be true
...
#### Scenario: Volatile read fast path for ready chunks

- **WHEN** `EnsureChunkReadyAsync` checks a chunk that is already extracted
- **THEN** it SHALL return immediately via a Volatile.Read check (no async await, no TCS access)

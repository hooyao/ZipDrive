# coalescing-batch-reader Specification

## Purpose
Defines requirements for the `CoalescingBatchCoordinator`, which batches concurrent memory-tier cache-miss requests for the same ZIP archive into a single sequential read pass to reduce I/O seeks.
...
- **THEN** defaults SHALL be: `Enabled=true`, `FastPathMs=0`, `WindowMs=500`, `DensityThreshold=0.8`, `SpeculativeCache=false`
